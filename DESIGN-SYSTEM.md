...
- **Impact tiles (`.cs-impact--tiles`):** one vertical stack of `.cs-impact-tile`s in the text
  column (`grid-column: 7/13`, same as `.cs-prose`/`.cs-quotes` above it — literally the same
  layout as `.cs-quotes` in Problem, at the user's request: `display: grid` + `row-gap` at
  desktop, `display: block` + `margin-top` on `+ .cs-impact-tile` at mobile, since `row-gap`
  doesn't survive the collapse to block there). Each tile is a `.cs-impact-lead` (30% smaller
  than the other `.cs-impact` variants' `--fs-xl`, at the user's request, now that five of these
  read as a stacked column rather than a grid) and a one-line `.cs-impact-body` in the muted
  colour, left-aligned, carrying the pull-quote rule (`.cs-quote`'s own `border-left`/
  `padding-left`, below), not a card — an earlier pass gave the tiles `.cs-finding-detail-card`'s
  look (background/radius/shadow/padding, centred text), then a two-row 4/6-column grid with the
  same rule but centred text; both replaced with this at the user's request. Usually a `from → to`
  figure (the arrow at 0.5625 of the figure size, weight 500, in `--color-text-muted`), but the
  lead can be a short evidence statement instead ("Same system, second product") when the tile is
  naming a kind of proof rather than a measured change — the two share one component; nothing
  distinguishes them but the lead's own content. No divider above the tiles. Used when a case has
  several approved figures; the lead stays below the Hook.
...
| `.cs-impact--tiles` | Impact | Five tiles, one vertical stack in the text column (7–12, same as `.cs-quotes` — see "Components" above): three before → after figures, then two evidence statements ("Same system, second product", "Shipped and stayed"). Left-aligned; each carries `.cs-quote`'s pull-quote rule (`border-left`, `--space-3` `padding-left`), not a card; lead 30% smaller than the other `.cs-impact` variants. |
