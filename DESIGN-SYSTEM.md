...
| `.cs-ia` | Solution | Grouped boxes: Assistant tools (columns 1–4), Business tools (5–9), and a smaller Account / utility group (10–12), the last with an outline and no shadow. Layout only, not a screenshot. Back in `.cs-body` right after the intro paragraph (its original position, and the same spot it returned to once the onboarding subsection that had briefly displaced it was deleted). |
