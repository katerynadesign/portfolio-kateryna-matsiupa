...
| `#design-system` | after the device showcase, before Evolution | A real subsection — title + divider and `.section` padding, the same rhythm as every named section — not one of the template's nine blocks and not listed in Contents, but it earns a heading because it's real reference content, not a placeholder loop (unlike the aria-hidden device showcase above it). Two `.cs-prose` paragraphs (what it is, then what the image below shows) introduce a real, full-width export (`design-system.png`: colour/opacity tokens, button anatomy with spacing annotations, button and input states) as `.cs-figure.cs-wide` — the page's ordinary contained-visual pattern, not `.cs-figure--bleed` — with a real `<img>`, not `.cs-media`, so it scales by width with `height: auto` (see ".cs-figure > img" above) and keeps its own aspect ratio. No `figcaption`: the description of what's shown lives in the second body paragraph instead, not under the image. |
