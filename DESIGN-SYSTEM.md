...
- **Header (`.nav`):** logo on the left, the nav links (or, on a phone, the menu toggle) on the
  ...
  - **Current page or section stays underlined** (`.nav-links a.is-active`, set by
    ...
    in — left on instead of triggered by the pointer. `work` is active only on `work.html`
    itself; a case study page is a branch off Work, not Work itself, so it shows no active nav
    item, the same as the Hero on the home page. On the home page, `about` and `contact` instead
    follow scroll position, via an `IntersectionObserver` on the two sections with a thin band
    across the viewport's middle 20% (`rootMargin: "-40% 0px -40% 0px"`) deciding which one
    counts as current; at the Hero,
...
- **Scroll-to-top (`.scroll-top`, shared partial `scroll-top.html` + `js/main.js`, every page):**
  fixed bottom-right (`--space-3` from each edge), 40×40px, `--radius` (2px, square rather than a
  pill), `--color-bg` fill (not `--color-surface` — a quiet control, not a card), `--shadow-card`
  only, no border. An upward arrow ("↑", the same plain-glyph approach as `.cs-impact-arrow`'s
  "→" — no icon font or SVG anywhere else on the site either). Hidden (`opacity`/`visibility`,
  not the `hidden` attribute, so it can fade) until the page's hero — `main > section`, the first
  one, whatever its own id/class (`#top` on the home page, `#hook` everywhere else) — scrolls out
  of the viewport (`IntersectionObserver`, the same technique the home page's active-nav tracking
  above already uses); hides again on scrolling back up into it. Click scrolls smoothly to top.
  Hover: fill steps up to `--color-surface`, the same one-step elevation language as everywhere
  else on the site.
...
- **Quick facts:** a definition list right after the Hook (role, engagement, team, tools),
  starting on column 1. Its width hugs its own content (`.cs-facts { grid-column: 1 / -1; width:
  fit-content; }`) rather than being forced to a fixed column span: it stays at its natural width
  — set by the longest value — as the viewport narrows, and only shrinks (then wraps) once the
  row itself is narrower than that. All values share one weight; no row is emphasised. Case study
  pages only — `work.html`'s Hook goes straight to Contents, since there is no single case's
  facts to show.
...
- **Text column:** prose sits in **columns 7–12** (6 of 12, `544px` on the 64px grid — moved
  from its original 5–10 at the user's request, to match `.section-content` on the home page;
  still 6 of 12 columns, so the same width and character count), which measures 65–75 characters
  per line in Karla 16px (all 17 measured lines, average 71.5). Seven columns measured 79–84 on
  the earlier 60px grid, which is too long. Cards, figures and any other wide element are
  unaffected — only `.cs-prose` moved; `.cs-figure`, `.cs-wide`, `.cs-row` etc. keep their own
  `grid-column` (below). Prose paragraphs and bullet lists use `--color-text-muted`, the same
  lighter tone as the Hook subtitle (6.04:1 on the page background, AA). Headings, labels, quotes
  and card titles stay `--color-text`.
- **Visual widths, outside the text column:** default = columns 5–12 (`.cs-figure`), wide =
  columns 1–12 (`.cs-wide`), full-bleed = edge to edge (`.cs-figure--bleed`) — note these start at
  column 5, one column to the left of where the (now columns 7–12) text column starts, matching
  how the Hook subtitle and other wide visuals have always related to the narrower prose above
  or below them.
...
- **Quick facts list (`.cs-facts-list`):** `dl` of `dt`/`dd` pairs, `dt` in uppercase muted
  `--fs-sm` (a label, not a button), hairline between rows. Each row is its own `6.5rem`-label +
  value grid (see "Quick facts" above for how the list as a whole is sized).
...
- **Impact (`.cs-impact`): one component, three variants.** `--metric` and `--evidence` use the
  same three slots and the same width as `.cs-prose` (columns 7–12, moved there along with it at
  the user's request), so every case looks alike. The template shows both; keep one per case and
  delete the other. `--tiles` (below the table) overrides this back to full width and shows
  several before → after figures side by side.
...
- **Impact tiles (`.cs-impact--tiles`):** `.cs-impact-tile`s across a 12-column grid, each a
  `--fs-xl` `.cs-impact-lead` and a one-line `.cs-impact-body` in the muted colour, left-aligned,
  top-aligned (`align-items: start`, so a tile's own box never gets stretched taller than its
  content). Usually a `from → to` figure (the arrow at 0.5625 of the figure size, weight 500, in
  ...
  two share one component and one row; nothing distinguishes them but the lead's own content.
  With five tiles (the AI Assistant Platform case): two rows, three tiles spanning 4 columns
  each on the first, two spanning 6 columns each on the second — 4+4+4 and 6+6 both fill 12
  exactly, so plain `grid-column: span N` per tile (`:nth-child(4)`/`(5)` get `span 6`, the rest
  `span 4`) wraps to a second row on its own, no explicit row assignment needed. Each tile carries
  the pull-quote rule (`.cs-quote`'s own `border-left`/`padding-left`, below), not a card — an
  earlier pass gave the tiles `.cs-finding-detail-card`'s look (background/radius/shadow/padding,
  centred text), replaced with this at the user's request; the row/span arrangement itself is
  unchanged from that pass. No divider above the tiles. They stack on mobile, one per row, full
  width (the mobile override has to repeat the `:nth-child(4)`/`(5)` selectors too: that pair's
  desktop span rule is equally specific and would otherwise still win at that width, since a
  media query changes nothing about specificity, only whether its own block applies at all).
  Used when a case has several approved figures; the lead stays below the Hook.
...
| `.cs-findings` | Discovery | The other four findings, numbered (`01`–`04`, CSS counter — starts over at the true count once "Content overload" is pulled out, nothing to renumber by hand — same text style as `.eyebrow`, reused verbatim since a pseudo-element can't carry a class), one per `.cs-row` column on desktop, stacked on a phone. Same stretch behaviour as the row above: on desktop the tallest card's front face (number, heading, body text — top-aligned) sets the row's height and the other cards stretch to match; on mobile each card hugs its own content, with `min-height` as a floor. Each card (`.cs-finding`, `--color-surface` with `--shadow-card`) flips to reveal a synthetic, illustrative quote on the back — deliberately separate from `.cs-quote` in Problem, which carries real, attributed research — no caption, centred rather than top-aligned; the flip is discoverable by touch/pointer alone. `.cs-finding-flip`, a `rotateY(180deg)` 3D transform with `backface-visibility: hidden`: only the back face is taken out of flow (`position: absolute`) to overlay the front — the front stays in normal flow specifically so its real content (not an arbitrary fixed box) is what sets the card's height; an earlier pass stacked both faces with `position: absolute` (or, briefly, a CSS grid stack), which left every card at the same bare `min-height` regardless of content since an out-of-flow face contributes nothing to layout, and the grid-stack alternative had its own bug — Chromium mis-rendered `backface-visibility` on grid-stacked faces, letting the back face's (mirrored) content bleed through on an unflipped card. Both faces are opaque so neither shows through the other, `transform: none` in reduced motion so the flip is instant, not animated, rather than suppressed. Triggered by `:hover`/`:focus`/`:focus-within` on a device with real hover, and by a tap-toggling `.is-flipped` class (`js/main.js`, checked at `click` time against `(hover: none)` — not once at load, which races an emulated or real touch device's capability settling) on one without. |
| `.cs-onboarding` | Process | The Onboarding flow, old and improved, two pairs of real images (below). Its `.cs-onb-title` ("onboarding") has no divider under it — `.cs-onb-title` had a `border-bottom` at first (Process only), then lost it there when Discovery's "content overload" (which reuses the same class, inside `.cs-findings-featured`, not `.cs-onboarding`) asked to lose its own divider too, so it's now off for both on the shared class rather than as two identical scoped overrides. Briefly reused verbatim for a second "onboarding" subsection in Solution (real product screenshots, `onboarding-real-before-*.png`/`onboarding-real-after-*.png`, instead of the Process diagrams) — hidden, then deleted at the user's request; the images are unreferenced now, kept on disk in case it's wanted again. Deleting it surfaced a sitewide fix worth keeping regardless: `.grid-12`'s own `display: grid` was silently overriding the browser's default `[hidden] { display: none }` while that subsection was hidden-not-deleted, since any author rule that sets `display` beats the UA stylesheet regardless of specificity — base.css now has `[hidden] { display: none !important; }` so the native `hidden` attribute works reliably everywhere on the site, not just where nothing else sets `display`. |
| `.cs-ia` | unused (was Solution) | Grouped boxes: Assistant tools (columns 1–4), Business tools (5–9), and a smaller Account / utility group (10–12), the last with an outline and no shadow. Layout only, not a screenshot. Replaced in the markup by a real IA diagram (below) at the user's request; the CSS stays in `css/case-study.css`, unused, kept in case a layout-only version is wanted again — nothing currently references `.cs-ia`. |
| IA diagram (`.cs-figure.cs-wide`) | Solution, right after the intro paragraph | `ia-architecture-diagram.png` (1877×622, a Home-rooted tree: Assistant tools / Business tools / Account & utility, each branch expanding into its own nested actions) as a full-width `.cs-figure.cs-wide` — the same contained-visual pattern as Design System's export below it, not `.cs-figure--bleed`. A real `<img>` (not `.cs-media`), so `.cs-figure > img`'s `width: 100%; height: auto` scales it by width only, preserving its own aspect ratio at every viewport — confirmed matching to four decimal places (rendered vs. natural ratio) rather than assumed. At a narrow phone width the 3:1 diagram's labels shrink along with it (unlike the two-tier `.cs-ia` boxes it replaced, which reflowed instead of shrinking) — flagged, not fixed, since a same-width image can't reflow the way boxes can. |
| `#design-system` | after Solution, before the device showcase | A real subsection — title + divider and `.section` padding, the same rhythm as every named section — not one of the template's nine blocks, but real enough content that it gets its own Contents entry on this page ("design system", between "solution" and "evolution", where it sits in the document) even though the template's placeholder case studies stick to nine. Two `.cs-prose` paragraphs (what it is, then what the image below shows) introduce a real, full-width export (`design-system.png`: colour and opacity tokens — now with hex values and a Background column alongside Text/Icon — button anatomy with spacing annotations, and button/input states) as `.cs-figure.cs-wide` — the page's ordinary contained-visual pattern, not `.cs-figure--bleed` — with a real `<img>`, not `.cs-media`, so it scales by width with `height: auto` (see ".cs-figure > img" above) and keeps its own aspect ratio. No `figcaption`: the description of what's shown lives in the second body paragraph instead, not under the image. |
| `.cs-device-showcase` | Solution, after Design System, at the end of the Solution area | Two transparent-background frame PNGs, `desktop-frame.png` and `phone-frame.png` (both 2000×1536, the same canvas, so they stack with `inset: 0` and no offset math), inside a wrapper carrying `aspect-ratio: 2000 / 1536` so nothing distorts. Two overlays, each sized (via `top`/`left`/`width`/`height`, not `inset: 0`) to its own frame's *actual screen slot* — not the full canvas — crossfade the real Lexi screenshots (`assets/images/projects/lexi-screens/`) inside each frame's actual screen, by measured percentages: 14 desktop shots in the monitor overlay, 16 mobile shots in the phone overlay (packets 01 and 06 each have two mobile variants: content/nav and chat-detail/chats-list), one panel per file in file order. Mobile screenshots are cropped from the top (`object-position: center bottom`), keeping each screen's bottom — its bezel-adjacent edge — in frame; the monitor keeps the default centred crop. Sizing the overlay itself to the slot (rounded with `overflow: hidden` + `border-radius`, not `clip-path`) matters: an earlier pass sized the overlay to the full canvas and only visually cropped it with `clip-path`, which doesn't change the layout box — every screenshot's `object-fit: cover` was cropping against that full, wrong-aspect-ratio box instead of the slot's own, so the monitor showed a tiny sliver of an over-zoomed crop and the phone (a tall, narrow slot) got its screenshot scaled against a wide landscape box.<br><br>Both overlays share one 16-unit cycle (`--device-panel-duration` × 16) so a packet always switches at the same instant on both frames. The phone's 16 panels are evenly timed, one unit each, `--i` (0-based file order) driving `animation-delay` directly. The monitor's 14 panels are *not* even: packets 01 and 06 have one desktop shot but two mobile ones, so those two desktop panels (`.cs-device-panel--double`, its own `@keyframes` with a wider pulse) hold for 2 units instead of 1 — 12×1 + 2×2 = 16, matching the phone exactly — and every other monitor panel's delay is a precomputed `--d` (not a simple `--i` formula, since the units are uneven); see the CSS comment for how `--d` is derived and recompute the whole column if a monitor panel is added, removed or reordered. Each panel's own pulse is wider than its unit by 0.175 units on each side (half of the original 0.35, for a deliberately snappier switch — half the fade time, more time held at full opacity), so the next one is already fading in before the current one reaches 0 — a continuous crossfade with no instant where every panel is at once transparent. `animation-delay` is negative (not positive): a positive delay would leave every panel but one sitting at its unanimated, blank `opacity: 0` for up to 15 panel-durations after page load; negative delay avoids that (the whole thing is already mid-cycle at first paint) but plays a panel's pulse *sooner* the larger its magnitude, so the phone's `--i` is inverted (`15 − --i`) to still advance in ascending file order. Both overlays freeze on their first screenshot (`:first-child`) under reduced motion. Stacking, bottom to top: desktop frame, the monitor's overlay, phone frame, the phone's overlay — the phone frame's own transparency means the monitor overlay needs no cut-out for it (unlike an earlier, photo-based pass: it painted over the phone's bezel there, fixed with a "keyhole" `clip-path` hole; a coded-CSS-bezel pass before that was replaced once real frame images existed). No section title: reinforces Solution rather than adding narrative, so it sits outside any `<section>` and is `aria-hidden`. Scales fluidly at every width, no breakpoint override needed. Superseded `.cs-devices` on this page, which is still used by the placeholder case studies (`.cs-device`, above). |
| `.cs-impact--tiles` | Impact | Five tiles, two rows (3×4 columns, then 2×6 — see "Components" above): three before → after figures, then two evidence statements ("Same system, second product", "Shipped and stayed"). Left-aligned, top-aligned; each carries `.cs-quote`'s pull-quote rule (`border-left`, `--space-3` `padding-left`), not a card. |
...
group titles label it and the Solution paragraph explains it. The Solution screens are now the real
`desktop-frame.png` / `phone-frame.png` device showcase (see the component, above); the one
